...
export default function PaymentPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false)
    if (!res.ok) { setError(data.error || "שגיאה בשליחה") } else { setSubmitted(true) }
  }
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "var(--background)" }}>
        <div style={{ textAlign: "center", background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 20, padding: 48, maxWidth: 440, boxShadow: "0 4px 24px rgba(99,102,241,0.08)" }}>
          ...
          <p style={{ color: "var(--muted)", lineHeight: 1.7, marginBottom: 24 }}>
            ...
            נאשר את הגישה שלך בהקדם האפשרי.<br />
            <strong style={{ color: "var(--foreground)" }}>בדרך כלל עד 24 שעות.</strong>
          </p>
          <button
            onClick={() => router.push("/dashboard")}
            style={{ padding: "12px 28px", background: "var(--primary)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: "pointer" }}
          >
            חזרה לדף הבית
          </button>
        </div>
      ...
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 20, padding: 32, marginBottom: 20, boxShadow: "0 4px 24px rgba(99,102,241,0.08)" }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <p style={{ fontWeight: 600, marginBottom: 12, fontSize: 15 }}>שלב 1 — העברה בביט:</p>
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--muted-bg)", border: "1px solid var(--card-border)", borderRadius: 10, padding: "12px 16px" }}>
              <span style={{ fontSize: 18, fontWeight: 700, flex: 1 }}>{PHONE_NUMBER}</span>
              <button
                onClick={copyPhone}
                style={{ padding: "6px 14px", background: copied ? "var(--success)" : "var(--primary)", color: "#fff", border: "none", borderRadius: 7, cursor: "pointer", fontSize: 13, fontWeight: 500, transition: "background 0.2s" }}
              >
                {copied ? "הועתק!" : "העתק"}
              </button>
            </div>
          ...
          <p style={{ fontWeight: 600, marginBottom: 16, fontSize: 15 }}>שלב 2 — שלח אישור תשלום:</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              <div key={key}>
                <label style={{ display: "block", marginBottom: 5, fontSize: 13, color: "var(--muted)" }}>{label}</label>
                <input
                  type={type}
                  ...
                  style={{ width: "100%", padding: "10px 14px", background: "var(--muted-bg)", border: "1px solid var(--card-border)", borderRadius: 10, color: "var(--foreground)", fontSize: 14, outline: "none" }}
                />
              </div>
            ))}
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ padding: "12px 24px", background: loading ? "var(--muted)" : "var(--primary)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", marginTop: 4 }}
            >
        ...
        {session?.user && (
          <p style={{ textAlign: "center", color: "var(--muted)", fontSize: 13 }}>מחובר כ: {session.user.email}</p>
        )}
      </div>
    </div>
  )
